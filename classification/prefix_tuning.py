...
def train_peft_model_w_prefix_tuning(
    ...
    """Train a classification model  using transformers. with prefix tuning in PEFT.
     The tokens of the input sequence can still attend to the prefix as virtual tokens.
     (Read https://arxiv.org/abs/2101.00190 to learn more about prefix tuning.)
    ...
    # PromptEncoder
    peft_config = PrefixTuningConfig(task_type=TaskType.SEQ_CLS, inference_mode=False,
    ...
    if use_wandb:
        import wandb
        wandb.init(project="adaptation-tuning-peft", name=folder_name, tags=['prefix_tuning'])
