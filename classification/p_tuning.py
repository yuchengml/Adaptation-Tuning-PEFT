...
def train_peft_model_w_p_tuning(
        train_csv_path: str,
        test_csv_path: str = None,
        pretrain_model: str = "prebuilt_model/chinese-roberta-wwm-ext",
    ...
    """Train a classification model using transformers. with p-tuning in PEFT.
     P-tuning uses a prompt encoder to optimize the prompt parameters.
     The tokens of the input sequence can still attend to the prompt as virtual tokens.
     (Read https://arxiv.org/abs/2103.10385 to learn more about p-tuning.)
    ...
    if use_wandb:
        import wandb
        wandb.init(project="adaptation-tuning-peft", name=folder_name, tags=['p-tuning'])
